...
func main() {
	...
	var server pb.MahasiswaServiceServer
	{
		...
		dbPwd := cfg.Get(cfg.DBpwd, "password.1")
		...
		service := svc.NewMahasiswa(dbReadWriter)
		endpoint := ep.NewMahasiswaEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCMahasiswaServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterMahasiswaServiceServer(grpcServer, server)
