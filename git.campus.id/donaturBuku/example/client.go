...
import (
	...
	cli "MiniProject/kampus.id/DonaturBuku/DonaturBuku/endpoint"
	//svc "git.bluebird.id/bluebird/util/server"
	opt "MiniProject/kampus.id/DonaturBuku/util/grpc"
	util "MiniProject/kampus.id/DonaturBuku/util/microservice"
...
func main() {
	...
	client, err := cli.NewGRPCDonaturBukuClient([]string{"127.0.0.1:2181"}, nil, option, tracer, logger)
	if err != nil {
	...
	*/
	...
	dbkEmail, _ := client.ReadCustomerByEmailService(context.Background(), "joko@gmail.com")
	fmt.Println("customer based on email:", dbkEmail)
}
